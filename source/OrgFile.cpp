...
typedef struct {
	unsigned short wait; //Speed of ORG
	unsigned char line; //Beat
	unsigned char dot; //Step
...
BOOL OrgData::SaveMusicData(void)
{
	...
	for(i=8;i<MAXTRACK;i++) //Checks for new Drams.
	{
...
BOOL OrgData::LoadMusicData(void)
{
	...
	/* //Was supposed to convert channels 9-16 to the drams of a vanilla ORG file.
	if (ver < 4)
	{
		i = nc.track1;
		for (i = MAXMELODY/2; i < MAXMELODY; i++)
		{
			InitDramObject(mi.tdata[nc.track1].wave_no, nc.track1 - MAXMELODY);
			if (!InitDramObject)
			{
				MessageBoxA(hWnd, "A Track couldn't be converted to it's Dram position.","Error", MB_OK);
			}
		}
	}
	*/
