...
//IDS_GRID_STRINGÇ…!ãÊêÿÇËÇ≈í«â¡Ç∑ÇÈÇ±Ç∆ÅBÇ±Ç±ÇÃï∂éöóÒÇÕ¿ﬁ–∞Ç≈Ç∑ÅB
...
//char *strTrack[] ={"ÇP", "ÇQ", "ÇR", "ÇS", "ÇT", "ÇU", "ÇV", "ÇW",  "Çp", "Çv", "Çd", "Çq", "Çs", "Çx", "Çt", "Çh"};	// 2014.10.19 D
char *strTrack[MAXTRACK];	// 2014.10.19 A
//char *strNNNTrack[] ={"[ÇP]", "[ÇQ]", "[ÇR]", "[ÇS]", "[ÇT]", "[ÇU]", "[ÇV]", "[ÇW]",  "[Çp]", "[Çv]", "[Çd]", "[Çq]", "[Çs]", "[Çx]", "[Çt]", "[Çh]"};	// 2014.10.19 D
char *strNNNTrack[MAXTRACK];	// 2014.10.19 A

int SamplePlayHeight = 36; //ÉTÉìÉvÉãâπÇñ¬ÇÁÇ∑çÇÇ≥

int SaveWithInitVolFile;	//ã»ÉfÅ[É^Ç∆ÅcÉZÅ[ÉuÇ∑ÇÈÇ©ÅB

extern HINSTANCE hInst;//ÉCÉìÉXÉ^ÉìÉXÉnÉìÉhÉã
extern void ClearEZC_Message(); //EZÉÅÉbÉZÅ[ÉWÇ∆îÕàÕÇè¡Ç∑
...
//	SetDlgItemText(hdwnd,IDD_SETTRACK,str);
	...
	for(i = 0; i < NUMGRIDA; i++){
		for(q = grid[i].name; *p != 0; p++, q++)*q = *p; //é¿éøstrcpy
		*q = 0; p++; //É|ÉCÉìÉ^Ç'\0'ÇÃéüÇ…
	}

	//ï\é¶ÉOÉäÉbÉhÇÃèâä˙âª//////////////////
	for(i = 0; i < NUMGRIDA; i++)//ÉäÉXÉgÉ{ÉbÉNÉXÇÃèâä˙âª
		SendDlgItemMessage(hdwnd,IDD_LB1,LB_ADDSTRING,0,(LPARAM)grid[i].name);
	for(i = 0; i < NUMGRIDA; i++)//åªç›égópíÜÇÃÉOÉäÉbÉhÇåüçı
		if(mi.line == grid[i].line && mi.dot == grid[i].dot)break;
	if(i == NUMGRIDA)i = 0;//ñ≥ÇØÇÍÇŒÇOÇê›íË
	...
	if(i > 0){
		...
		EnableWindow(GetDlgItem(hdwnd, IDD_GRIDEDIT1), FALSE); //é©óRê›íËÉeÉLÉXÉgñ≥å¯âª
		EnableWindow(GetDlgItem(hdwnd, IDD_GRIDEDIT2), FALSE);
	}else{
		EnableWindow(GetDlgItem(hdwnd, IDD_GRIDEDIT1), TRUE); //é©óRê›íËÉeÉLÉXÉgóLå¯âª
	...
	//MessageBox(NULL, "ÉÅÉbÉZÅ[ÉWÉãÅ[ÉvÇî≤ÇØÇ‹ÇµÇΩ", "OK", MB_OK);
}
...
BOOL SetGrid(HWND hdwnd,MUSICINFO *mi)
{
	int i;
	//ëIëÉOÉäÉbÉhÇÃê›íË///////////////////////
	i = SendDlgItemMessage(hdwnd, IDD_LB1,LB_GETCURSEL,0,0);//ÉCÉìÉfÉbÉNÉXÇìæÇÈ
//	EndDialog(hdwnd,0);
	if(i>0){
		mi->dot = grid[i].dot;
		mi->line = grid[i].line;
...
BOOL CALLBACK DialogSetting(HWND hdwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	int iWAIT;
	char str[128] = {NULL};
	RECT rect = {64,0,WWidth,WHeight};//çXêVÇ∑ÇÈóÃàÊ
	MUSICINFO mi,mg;
	switch(message){
	case WM_INITDIALOG://É_ÉCÉAÉçÉOÇ™åƒÇŒÇÍÇΩ
		...
		return 1;
	case WM_CTLCOLOREDIT: //ÉeÉLÉXÉgÇ™ï“èWÇ≥ÇÍÇΩÅB
	...
	case WM_COMMAND:
		...
		case IDC_BTN2:
			...
			return 1;
		case IDD_LB1:
			if(HIWORD(wParam) == LBN_SELCHANGE){ //ÿΩƒŒﬁØ∏ΩÇ≈ÇÃëIëïœçX
				i = SendDlgItemMessage(hdwnd, IDD_LB1,LB_GETCURSEL,0,0);//ÉCÉìÉfÉbÉNÉXÇìæÇÈ
				if(i == 0){
		...
		case IDD_SETFREQ8: case IDD_SETFREQ9: case IDD_SETFREQ10: case IDD_SETFREQ11: case IDD_SETFREQ12: case IDD_SETFREQ13: case IDD_SETFREQ14: case IDD_SETFREQ15:
			if(HIWORD(wParam) == EN_SETFOCUS)PostMessage(GetDlgItem(hdwnd, LOWORD(wParam)), EM_SETSEL, 0, -1); //ÉtÉHÅ[ÉJÉXéûÇ…ÉeÉLÉXÉgÇëSëIëÇ∑ÇÈ
			return -1;
		case IDCANCEL:
			EndDialog(hdwnd,0);
		...
		case IDOK:
			...
			EndDialog(hdwnd,0);
			EnableDialogWindow(TRUE);
			ClearEZC_Message(); //EZÉÅÉbÉZÅ[ÉWÇ∆îÕàÕÇè¡Ç∑
			return 1;
		}
	}
	return 0;
...
BOOL CALLBACK DialogWave(HWND hdwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
	case WM_INITDIALOG://É_ÉCÉAÉçÉOÇ™åƒÇŒÇÍÇΩ
		//strTrack, strNNNTrackÇ…ï∂éöóÒÇë„ì¸Ç∑ÇÈÅB
		for (p = MessageString[IDS_STRING112], i = 0; i < MAXTRACK; i++) {
			strTrack[i] = (char*)p;
			for (; *p != 0; p++); //ï∂éöóÒèIí[Ç‹Ç≈É|ÉCÉìÉ^à⁄ìÆ
			p++; //ÇªÇÃéüÇÃï∂éöÇ…É|ÉCÉìÉ^à⁄ìÆ
		}
		...
		//ÉÅÉçÉfÉBÉäÉXÉgÉ{ÉbÉNÉXÇÃèâä˙âª
		for (j = 0; j < MAXMELODY; j++) {
			SendDlgItemMessage(hdwnd, freqbox[j], LB_RESETCONTENT, 0, 0);//èâä˙âª
			for (i = 0; i < MAXWAVE; i++) {
		...
		Sl_Reset(hdwnd); //ÉXÉâÉCÉ_Å[èâä˙âª
		EnableDialogWindow(FALSE);
		iLastLBox = 0;
	...
	case WM_COMMAND:
		if ((LOWORD(wParam) >= IDD_SETFREQx0 && LOWORD(wParam) <= IDD_SETFREQ15) && (HIWORD(wParam) == EN_SETFOCUS)) {	// 2014.10.19 
			PostMessage(GetDlgItem(hdwnd, LOWORD(wParam)), EM_SETSEL, 0, -1); //ÉtÉHÅ[ÉJÉXéûÇ…ÉeÉLÉXÉgÇëSëIëÇ∑ÇÈ
			return -1;
		}
		...
		else if ((HWND)lParam == GetDlgItem(hdwnd, VS_SLIDER_RESET)) { //ÉäÉZÉbÉgÉ{É^Éì
		   //FREQ & PIPI
			SamplePlayHeight = 36;
			Sl_Reset(hdwnd);
			iMeloDrumMode = 0;
		...
		for (j = 0; j < MAXMELODY; j++) {
			if (freqbox[j] == LOWORD(wParam)) {
				if (HIWORD(wParam) == LBN_SELCHANGE) {
					...
					Rxo_StopAllSoundNow();
					i = SendDlgItemMessage(hdwnd, freqbox[j], LB_GETCURSEL, 0, 0);//ÉCÉìÉfÉbÉNÉXÇìæÇÈ
		...
		for (j = MAXMELODY; j < MAXTRACK; j++) {
			if (freqbox[j] == LOWORD(wParam)) {
				if (HIWORD(wParam) == LBN_SELCHANGE) {
					...
					Rxo_StopAllSoundNow();
					i = SendDlgItemMessage(hdwnd, freqbox[j], LB_GETCURSEL, 0, 0);//ÉCÉìÉfÉbÉNÉXÇìæÇÈ
					i = Wave_no_to_List_no[i]; //ÉAÉãÉtÉ@ÉxÉbÉgèá(Listèá)Ç©ÇÁWaveèáÇ…ïœä∑Ç∑ÇÈÅBÇ±ÇÍèdóvÅB
...
bool GetSetPVInitFile(HWND hdwnd, int IsSave, int iQuiet)
{
	...
	ofn.hwndOwner   = hdwnd;
	ofn.hInstance   = hInst;
	//ofn.lpstrFilter = "ÉpÉìÅ^É{ÉäÉÖÅ[ÉÄèâä˙ílÉfÅ[É^[*.pvi]\0*.pvi\0ëSÇƒÇÃå`éÆ [*.*]\0*.*\0\0"; //ÉpÉìÅ^É{ÉäÉÖÅ[ÉÄèâä˙ílÉfÅ[É^[*.pvi]\0*.pvi\0ëSÇƒÇÃå`éÆ [*.*]\0*.*\0\0	// 2014.10.19 D
	ofn.lpstrFilter = MessageString[IDS_STRING108]; //ÉpÉìÅ^É{ÉäÉÖÅ[ÉÄèâä˙ílÉfÅ[É^[*.pvi]\0*.pvi\0ëSÇƒÇÃå`éÆ [*.*]\0*.*\0\0	// 2014.10.19 A
	...
	if(IsSave>0){ //ÉZÅ[Éu
		...
		ofn.lpstrTitle  = MessageString[IDS_STRING65]; //ÉpÉìÅ^É{ÉäÉÖÅ[ÉÄèâä˙ílÉfÅ[É^ÇÃï€ë∂
		if(GetSaveFileName(&ofn));	//InvalidateRect(hwnd,NULL,TRUE);
		else return false;			//ÉLÉÉÉìÉZÉãÇ≈0Ç™ï‘ÇÈ
		
	}else{			//ÉçÅ[Éh
		ofn.Flags       = OFN_HIDEREADONLY;
		ofn.lpstrTitle  = MessageString[IDS_STRING66]; //ÉpÉìÅ^É{ÉäÉÖÅ[ÉÄèâä˙ílÉfÅ[É^ÇÃì«Ç›çûÇ›
		if(GetOpenFileName(&ofn));	//InvalidateRect(hwnd,NULL,TRUE);
		else return false;			//ÉLÉÉÉìÉZÉãÇ≈0Ç™ï‘ÇÈ
	}
	fp = fopen(init_file,"rb");		//åüç∏
	if(fp == NULL){
		if(IsSave==0){
			if(iQuiet == 0) MessageBox(hdwnd,"Initial PVI file does not exist!!","Error(PVI)",MB_OK);	// 2014.10.19 D
			return false;			//éwíËÉtÉ@ÉCÉãÇ™ë∂ç›ÇµÇ»Ç¢
		}
	}else{
		fclose(fp);
	}
...
BOOL CALLBACK DialogDefault(HWND hdwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_VOLUMEDxQ: case IDC_VOLUMEDxW: case IDC_VOLUMEDxE: case IDC_VOLUMEDxR: case IDC_VOLUMEDxT: case IDC_VOLUMEDxY: case IDC_VOLUMEDxU: case IDC_VOLUMEDxI: case IDC_VOLUMEDxA: case IDC_VOLUMEDxS: case IDC_VOLUMEDxD: case IDC_VOLUMEDxF: case IDC_VOLUMEDxG: case IDC_VOLUMEDxH: case IDC_VOLUMEDxJ: case IDC_VOLUMEDxK:
			if(HIWORD(wParam) == EN_SETFOCUS)PostMessage(GetDlgItem(hdwnd, LOWORD(wParam)), EM_SETSEL, 0, -1); //ÉtÉHÅ[ÉJÉXéûÇ…ÉeÉLÉXÉgÇëSëIëÇ∑ÇÈ	// 2014.10.19 
			return 1;
		...
		case IDC_PRESET5: //Reset
			...
			break;
		case IDOK:
			if( !CheckDefPanVol(hdwnd, 1) )return FALSE; //åüç∏Ç∆ÉZÉbÉeÉBÉìÉO
			EndDialog(hdwnd,0);
			EnableDialogWindow(TRUE);
			return 1;
		}
...
{
//	char str[10] = {NULL};
//	char HelpString[10000]; //Ç∆ÇËÇ†Ç¶Ç∏10kB
	char *HelpString; //A 2010.9.22 ÉwÉãÉvÉtÉ@ÉCÉãÇÃóeó ägëÂÇ…î∫Ç¢ÅAìÆìIämï€Ç…ïœçXÅB
	HRSRC hrscr;
	HWND haDlg;
	MINMAXINFO *pmmi;
	//DWORD *lpdword;//ÉäÉ\Å[ÉXÇÃÉAÉhÉåÉX
	char *cc,*ce,*cf;
	//int i;
	...
	case WM_INITDIALOG://É_ÉCÉAÉçÉOÇ™åƒÇŒÇÍÇΩ
		t = 0;
		HelpString = (char *)malloc(1048576 * sizeof(char));//ìÆìIóeó ämï€(Ç∆ÇËÇ†Ç¶Ç∏1MB) A 2010.09.22
		//EnableDialogWindow(FALSE); //D 2014.05.25
		hrscr = FindResource(NULL, "TEXTHELP" , "TEXT");// ÉäÉ\Å[ÉXÇÃåüçı
		cc = (char*)LockResource(LoadResource(NULL, hrscr));// ÉäÉ\Å[ÉXÇÃÉAÉhÉåÉXÇéÊìæ
		cf = HelpString; ce = cc;
		do{
			if(*ce=='\n'){ //â¸çsÉRÅ[Éhïœä∑ÅiÉâÉCÉìÉtÉBÅ[ÉhÇâ¡Ç¶Ç»Ç¢Ç∆Ç¢ÇØÇ»Ç¢ÇÁÇµÇ¢Åj
				*cf='\r';
				cf++;
			}
			*cf = *ce;
		...
		*cf = *ce;
		...
		free(HelpString); //äJï˙ A 2010.09.22
		...
		return 1;
	case WM_SIZE:
		DlgWidth  = LOWORD(lParam);	//ÉNÉâÉCÉAÉìÉgóÃàÊÇÃÉTÉCÉY
